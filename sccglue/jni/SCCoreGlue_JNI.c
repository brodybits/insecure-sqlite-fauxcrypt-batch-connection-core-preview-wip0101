/* !---- DO NOT EDIT: This file autogenerated by com/jogamp/gluegen/JavaEmitter.java on Fri May 15 01:15:09 EDT 2020 ----! */
...
/*   Java->C glue code:
 *   Java package: io.sqlc.SCCoreGlue
 *    Java method: int scc_open_connection(java.lang.String full_name, int flags)
 *     C function: int scc_open_connection(const char *  full_name, int flags);
 */
JNIEXPORT jint JNICALL
Java_io_sqlc_SCCoreGlue_scc_1open_1connection__Ljava_lang_String_2I(JNIEnv *env, jclass _unused, jstring full_name, jint flags) {
  const char* _strchars_full_name = NULL;
  int _res;
  if ( NULL != full_name ) {
    _strchars_full_name = (*env)->GetStringUTFChars(env, full_name, (jboolean*)NULL);
  if ( NULL == _strchars_full_name ) {
      (*env)->ThrowNew(env, (*env)->FindClass(env, "java/lang/OutOfMemoryError"),
                       "Failed to get UTF-8 chars for argument \"full_name\" in native dispatcher for \"scc_open_connection\"");
      return 0;
    }
  }
  _res = scc_open_connection((char *) _strchars_full_name, (int) flags);
  if ( NULL != full_name ) {
    (*env)->ReleaseStringUTFChars(env, full_name, _strchars_full_name);
  }
  return _res;
}
